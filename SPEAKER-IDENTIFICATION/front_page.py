...
PAGE_HIGHT = 700
PAGE_WIDTH = 1100
PATH = r"F:\4th year\7 sem\CP441\project\github\SPEAKER-IDENTIFICATION\DATASET"
DATA_PATH = r"DATASET"
...
def open_file(event):
    ...
    if event.widget.cget("text") == 'Click to Record1':
        record(0)
        flag_list[0] = 1
        file_name[0].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record2':
        record(1)
        flag_list[1] = 1
        file_name[1].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record3':
        record(2)
        flag_list[2] = 1
        file_name[2].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record4':
        record(3)
        flag_list[3] = 1
        file_name[3].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record5':
        record(4)
        flag_list[4] = 1
        file_name[4].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record6':
        record(5)
        flag_list[5] = 1
        file_name[5].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record7':
        record(6)
        flag_list[6] = 1
        file_name[6].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record8':
        record(7)
        flag_list[7] = 1
        file_name[7].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record9':
        record(8)
        flag_list[8] = 1
        file_name[8].set('Finished recording')
    elif event.widget.cget("text") == 'Click to Record10':
        record(9)
        flag_list[9] = 1
        file_name[9].set('Finished recording')
    if event.widget.cget("text") == 'Click to Record':
        record(10)
...
def open_file1(event):
    ...
    file1 = filedialog.askopenfilename( title="Select file", filetypes=(("wav files", "*.wav"),))
    if file1 == '':
        return
    if event.widget.cget("text") == 'Choose file1':
        ...
        flag_list[4] = 1
    elif event.widget.cget("text") == 'Choose file6':
        file_name[5].set(file1)
        flag_list[5] = 1
    elif event.widget.cget("text") == 'Choose file7':
        file_name[6].set(file1)
        flag_list[6] = 1
    elif event.widget.cget("text") == 'Choose file8':
        file_name[7].set(file1)
        flag_list[7] = 1
    elif event.widget.cget("text") == 'Choose file9':
        file_name[8].set(file1)
        flag_list[8] = 1
    elif event.widget.cget("text") == 'Choose file10':
        file_name[9].set(file1)
        flag_list[9] = 1
    elif event.widget.cget("text") == 'Choose file':
        test_file_name.set(file1)
        submit_test['state'] = NORMAL
...
def submit():
    ...
    for name, i, audio in zip(file_name, range(1, 11), audio_list):
...
def submit1():
    ...
    for name, i in zip(file_name, range(1, 11)):
        set_path = os.path.join(set_data_path, f"{user_entry.get()}{i}.wav")
...
def test():
    ...
    speck_fun(f"given audio clip is from {result}")

    I += 1


def test1():
    global test_audio, I, result_var, test_file_name
    set_path = os.path.join(SAMPLE_PATH, f"test{I}.wav")
    shutil.copyfile(test_file_name.get(), set_path)
    result = test_model(f"test{I}.wav")
    result_var.set(result)
    speck_fun(f"given audio clip is from {result}")
...
def ragister_page_record():
    ...
    user_name_text = Entry(main_frame, textvariable=user_entry, font="Arial 25", width=10)
    user_name_text.grid(row=0, column=1, padx=10, pady=10)


    for i in range(1, 6):
        open_file_button = Button(main_frame, text=f"Click to Record{i}", background=BBG, font="Arial 17", width=13)
    ...
    for i in range(1, 6):
        open_file_button = Button(main_frame, text=f"Click to Record{i+5}", background=BBG, font="Arial 17", width=13)
        open_file_button.grid(row=i, column=2, padx=10, pady=10)
        open_file_button.bind("<Button-1>", open_file)

        file_name_label = Label(main_frame, textvariable=file_name[i-1+5], font="Arial 12", background=MFBG)
        file_name_label.grid(row=i, column=3, padx=10, pady=10, sticky=W)
...
def ragister_page_file():
    ...
    for i in range(1, 6):
        open_file_button = Button(main_frame, text=f"Choose file{i+5}", background=BBG, font="Arial 17", width=13)
        open_file_button.grid(row=i, column=2, padx=10, pady=10)
        open_file_button.bind("<Button-1>", open_file1)
...
def test_page():
    global main_frame, test_frame, user_entry, file_name, submit_button
    try:
        main_frame.destroy()
    except:
        pass
    ...
    main_frame = Frame(test_frame, background=BG)

    file_button = Button(main_frame, text="By File", font="Arial 17", width=13, background=BBG,
                         command=test_page_file)
    file_button.grid(row=0, column=0, padx=10, pady=10)

    voice_button = Button(main_frame, text="By Voice", font="Arial 17", width=13, background=BBG,
                          command=test_page_record)
    voice_button.grid(row=0, column=1, padx=10, pady=10)
...
def test_page_record():
    ...
    open_file_button = Button(button_frame, text=f"Click to Record", background=BBG, font="Arial 17", width=13)
...
def test_page_file():
    ...
    submit_test = Button(button_frame, text="Check", font="Arial 17", width=13, background=BBG, state=DISABLED, command=test1)
    submit_test.grid(row=1, column=0, padx=10, pady=10)
